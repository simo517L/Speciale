...
#This function returns the evaluation value for the clustes made from a divisive alg.  
eval_clust_diana = function(distmatrix,k,true_label,method=T){
  ...
  hc = diana(distmatrix,diss = method)
  ...
  return(c(hc$dc,randindex,F_measure)) 
}
path = "C:/Users/simon/Desktop/SpecialeProject/Speciale/Data" # Remember yo set your own path 
setwd(path) 
...
logpath = "C:/Users/simon/Desktop/SpecialeProject/Speciale/logHCtxt" # Remember to add your own logpath 
path_of_log<-file(logpath)

name1 ="eval1st_diana.Rdata"
name2 ="eval2st_diana.Rdata"
name3 ="eval3st_diana.Rdata"

registerDoParallel(10) # We define the number of cores we want to use
...
hceval1 <- foreach (i= c(1:10), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
  ...
  eval_clust(MM,k=3,true_label =TrueLabel,method=T)
}
saveRDS(hceval1,file = name1)
for(j in c(2:50)){
  setwd(path) 
  hceval1temp <- foreach (i= c(intval[j]:(intval[j+1]-1)), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
    vec = c((i*5-4):(i*5))
    MM  = distMppp(c(Data[vec],Matern4b[vec],Clust4a[vec]),method=3 ,minpoints=5)
    eval_clust(MM,k=3,true_label =TrueLabel,method=T)
  }
...
close(path_of_log)
path_of_log<-file(logpath)
hceval2 <- foreach (i= c(1:10), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
  vec = c((i*5-4):(i*5))
...
for(j in c(2:50)){
  setwd(path) 
  hceval2temp <- foreach (i= c(intval[j]:(intval[j+1]-1)), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
    vec = c((i*5-4):(i*5))
    MM  = distMppp(c(Data[vec],Matern4a[vec],Clust4c[vec]),method=3 ,minpoints=5)
    eval_clust(MM,k=3,true_label =TrueLabel,method=T)
  }
...
hceval3 <- foreach (i= c(1:10), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
  vec = c((i*3-2):(i*3))
  MM  = distMppp(c(Data[vec],Matern4a[vec],Matern4b[vec],Clust4a[vec],Clust4b[vec],Clust4c[vec]),method=3 ,minpoints=5)
  eval_clust(MM,k=6,true_label =TrueLabel,method=T)
}
saveRDS(hceval3,file = name3)
for(j in c(2:50)){
  setwd(path) 
  hceval3temp <- foreach (i= c(intval[j]:(intval[j+1]-1)), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
    vec = c((i*3-2):(i*3))
    MM  = distMppp(c(Data[vec],Matern4a[vec],Matern4b[vec],Clust4a[vec],Clust4b[vec],Clust4c[vec]),method=3 ,minpoints=5)
    eval_clust(MM,k=3,true_label =TrueLabel,method=T)
  }
