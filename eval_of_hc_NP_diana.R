...
logpath = "C:/Users/simon/Desktop/SpecialeProject/Speciale" # Remember to add your own logpath 
path_of_log<-file(logpath)

name1 ="eval1dianaTT.Rdata"
name2 ="eval2diana.Rdata"
name3 ="eval3diana.Rdata"
registerDoParallel(3) # We define how many cores we want to use. 
m=10
intval = seq(1,m,10)
...
hceval1 <- foreach (i= c(1:10), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
  ...
  eval_clust_diana(MM,k=3,true_label =TrueLabel,method=T)
}
...
hceval3 <- foreach (i= c(1:10), .combine="cbind", .packages = c("spatstat","cluster","fossil","ppMeasures")) %dopar% {
  ...
  eval_clust_diana(MM,k=6,true_label =TrueLabel,method=T)
}
