...
setwd(path) 
powertest_OF_sq = function(Outlier,Data,name,n=NULL,m,squares,newlog = F,DataSize,method=1,Kinterval = c(5:10),path){
  ...
  if (is.null(n)){
    ...
    ResultpowerM1temp1 <- foreach (i= c(1:m), .combine="cbind", .packages = c("spatstat","ppMeasures")) %:%
      foreach (j= c(1:length(squares)), .combine="c", .packages = c("spatstat","ppMeasures")) %dopar% {
...
powertest_OF_sq(Outlier = Matern4a,Data=Data,name ="Power_MaternA_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20,newlog=T,path=path )

Matern4b = readRDS(file = "Matern_b.Rdata")
powertest_OF_sq(Outlier = Matern4b,Data=Data,name ="Power_MaternB_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20,path=path )

Clust4a = readRDS(file = "Clust_a.Rdata")
powertest_OF_sq(Outlier = Clust4a ,Data=Data,name ="Power_ClusterA_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20 ,path=path )

Clust4b = readRDS(file = "Clust_b.Rdata")
powertest_OF_sq(Outlier = Clust4b ,Data=Data,name ="Power_ClusterB_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20,path=path )

Clust4c = readRDS(file = "Clust_c.Rdata")
powertest_OF_sq(Outlier = Clust4c ,Data=Data,name ="Power_ClusterC_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20,path=path  )


poistest  = readRDS(file = "poisPPP.Rdata")
powertest_OF_sq(Outlier = poistest ,Data=Data,name ="pois_OFsq.Rdata",n=1000,method = 1,m=mm,squares = squares,DataSize=20,path=path )
